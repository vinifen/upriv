...
use crate::contents::{probe_unlock_preset, KdfUnlockPreset};
...
fn list_item_from_entry(entry: &VaultListEntry) -> VaultListItem {
    let last_accessed_at = persistence_last_accessed(&entry.vault_dir, &entry.id);
    let unlock_preset = probe_unlock_preset(entry.vault_dir.join("contents"));
...
pub fn list_vaults(root: &VaultRoot) -> Result<Vec<VaultListItem>> {
    ...
    Ok(entries.iter().map(list_item_from_entry).collect())
}
...
pub fn vault_list_item(root: &VaultRoot, vault_id: &str) -> Result<VaultListItem> {
    ...
    Ok(list_item_from_entry(&VaultListEntry {
        id: config.vault.id.clone(),
        display_name: config.vault.display_name.clone(),
        order: config.vault.order,
        note: config.vault.note.clone(),
...
"#
    ...
    fn create_open_close_empty_vault() {
        ...
        let contents = root.vault_contents_dir("notes").unwrap();
        let opened = crate::contents::open_store(&contents, b"pass-word-ok").expect("decrypt");
        let expected = crate::contents::seed_plaintext(opened.header.content_identity);
        let got = crate::contents::read_logical_file(
            &contents,
            &opened.header,
            &opened.content_key,
            &opened.index,
            crate::contents::SEED_LOGICAL_PATH,
        )
        .expect("seed");
...
"#,
    ...
    fn open_store_bypasses_surface_throttle() {
        ...
        let contents = root.vault_contents_dir("notes").unwrap();
        assert!(
            crate::contents::open_store(&contents, b"nope-nope-n").is_err(),
            "still wrong password"
        );
        crate::contents::open_store(&contents, b"pass-word-ok")
