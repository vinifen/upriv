...
export function VaultGroupRow({
  ...
  return (
    <article
      className={[
        // Vertical pad on toggle/sides — article `py-*` leaves dead bands in large mode.
        "flex items-stretch justify-between gap-3 bg-transparent px-3 sm:pr-4",
    ...
    >
      {!dragDisabled ? (
        <div className={["flex shrink-0 items-center", density.article].join(" ")}>
          <VaultDragHandle
      ...
      <div
        ...
        className={[
          "flex min-w-0 flex-1 cursor-pointer items-center gap-2 sm:gap-3",
          density.article,
        ].join(" ")}
      ...
      >
        ...
        >
          <Icon
            ...
            style={{
              transitionDuration: `${group.collapsed ? GROUP_COLLAPSE_MS : GROUP_EXPAND_MS}ms`,
            }}
          />
        </span>
